...
def __main__():
    ...
    flt_na = flt_da.to_n_alt(fill=-1)

    # update variant index
    pos = pos[flt_mask]

    print('Calculating LD in windows: ')
    ...
    for i, region in bed.iterrows():
        ...
        R2 = ska.stats.rogers_huff_r(flt_na[loc_region, :]) ** 2
        region_name = '{}_{}_{}'.format(region['chr'], region['start'], region['stop'])
        np.save(os.path.join('data/', region_name + '.R2'), R2)
